...
| fd, addr, port -> peers |> List.filter (fun (_, a, ip) -> addr ^ ":" ^ port <> a ^ ":" ^ ip) |> List.append [(fd, addr, port)]

let remove_from_peers prs addr port = prs |> List.filter (fun (_, a, ip) -> addr ^ ":" ^ port <> a ^ ":" ^ ip)
...
let connect_to_peer_on_join hostname port = 
  ...
  Lwt.return (peers := add_to_peers !peers (socket, hostname, port)) >>= fun _ ->
  Logs_lwt.info (fun m -> m "connect to %s:%d" hostname iport)
...
let connect_to_rest_peer_impl hostname port =
  ...
  Lwt_unix.connect socket sockaddr >>= fun () ->
  Lwt.return (peers := add_to_peers !peers (socket, hostname, port)) >>= fun _ ->
  send_message socket ("join 127.0.0.1 " ^ Sys.argv.(1)) >>= fun () ->
  recieve_response socket >>= fun _ ->
  Logs_lwt.info (fun m -> m "connect to %s:%d" hostname iport)
...
let rec connect_to_rest_peers_impl = function
  | [] -> Lwt.return_unit
  | head :: tail ->
      Logs_lwt.info (fun m -> m "connect to rest %s" head) >>= fun () ->
...
let connect_via_peer host port =
  ...
  Lwt_unix.setsockopt_float socket SO_RCVTIMEO timeout;
  Lwt_unix.connect socket sockaddr >>= fun () ->
  Lwt.return (peers := add_to_peers !peers (socket, host, port)) >>= fun _ ->
  send_message socket ("join 127.0.0.1 " ^ Sys.argv.(1)) >>= fun () ->
  recieve_response socket >>= fun response ->
  connect_to_rest_peers response >>= fun () ->
  send_message socket "history" >>= fun () ->
  recieve_response socket >>= fun response -> 
  Lwt.return (Str.split (Str.regexp "[;]+") response |> List.filter (fun x -> x != "")) >>= fun msgs ->
  let rec print_msg = function
    | [] -> Lwt.return_unit
    | head :: tail -> Logs_lwt.info (fun m -> m "%s" head) >>= fun () -> print_msg tail
  in 
  print_msg msgs
...
let connect_via_discovery host port =
  ...
  recieve_response socket >>= fun response ->
  connect_to_rest_peers response >>= fun () ->
  if List.length !peers < 2 then
    Lwt.return_unit
  else
    let sock, _, _ = List.nth !peers 0 in
    send_message sock "history" >>= fun () ->
    recieve_response sock >>= fun response -> 
    Lwt.return (Str.split (Str.regexp "[;]+") response |> List.filter (fun x -> x != "")) >>= fun msgs ->
    let rec print_msg = function
    | [] -> Lwt.return_unit
    | head :: tail -> Logs_lwt.info (fun m -> m "%s" head) >>= fun () -> print_msg tail
...
| [] -> Lwt.return_unit
| head :: tail -> 
  send_msg_to_peer msg head >>= fun () -> 
  Logs_lwt.info (fun m -> m "sending %s" msg) >>= fun () ->
  send_msg_to_peers msg tail
...
| "\\q" -> close_peers !peers
| msg -> history := add_to_history ("[127.0.0.1:" ^ Sys.argv.(1) ^ "] username: " ^ msg) !history; send_msg_to_peers msg !peers
...
let get_text cmd host port username msg =
  let pos = 4 + String.length cmd + String.length host + String.length port + String.length username in
  String.sub msg pos (String.length msg - pos)

let handle_message fd msg = 
  match Str.split (Str.regexp "[ \t\n\r\x0c]+") msg with
  | "join" :: host :: port :: [] -> peers := add_to_peers !peers (fd, host, port) ; connect_to_peer_on_join host port >>= fun () -> Lwt.return (peers_as_string (List.filter (fun (_, x_host, x_port) -> x_host ^ ":" ^ x_port <> host ^ ":" ^ port) !peers))
  | "msg" :: host :: port :: username :: _-> let text = get_text "msg" host port username msg in history := add_to_history ("[" ^ host ^ ":" ^ port ^ "] " ^ username ^ ": " ^ text) !history; Logs_lwt.info (fun m -> m "%s" text) >>= fun () -> Lwt.return "accepted"
  | "history" :: [] -> Lwt.return (history_as_string !history)
  | "close" :: host :: port :: [] -> peers := remove_from_peers !peers host port ; Lwt.return "exit"
...
let rec handle_connection fd ic oc () =
  Lwt_io.read_line_opt ic >>=
    (fun msg ->
      match msg with
      | Some msg -> 
        handle_message fd msg >>= fun reply ->
        Logs_lwt.info (fun m -> m "request=%s response=%s " msg reply) >>= fun () ->
        Lwt_io.write_line oc reply >>= fun() -> Lwt_io.flush oc >>= handle_connection fd ic oc
      | None ->
...
let accept_connection conn =
  ...
  let ic = Lwt_io.of_fd ~mode:Lwt_io.input fd in
  let oc = Lwt_io.of_fd ~mode:Lwt_io.output fd in
  Lwt.on_failure (handle_connection fd ic oc ()) 
    (fun e -> 
...
let create_server_socket port =
  ...
  Lwt_main.run (Logs_lwt.info (
    fun m -> m "Starting on host = %s ; port = %d" (Unix.string_of_inet_addr listen_address) port
  ) >>= return);
